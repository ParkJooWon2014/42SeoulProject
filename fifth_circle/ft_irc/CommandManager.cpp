...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/03 15:00:33 by joowpark          #+#    #+#             */
/*   Updated: 2023/03/06 18:45:53 by joowpark         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int CommandManager::doCommand(string line, int fd)
{
	...
	vector<string> parsed=parseCmd(line);
	...
	{
		if(parsed[0]=="JOIN")
			ret = command_join(client, parsed);
...
int CommandManager::command_join(Client& client, vector<string>& parsed)
{
	...
	for(size_t i=1; i<parsed.size(); i++)
	{
		...
		{
			// create new channel
			if(chan_name[0]!='#' or chan_name.find("\x07") != string::npos)
			{
...
int CommandManager::command_mode(Client& client, vector<string>& parsed)
{
	...
	{
		if(!channels[chan_name]->getClients()[&client])
...
int CommandManager::register_process(vector<string>& parsed, Client& client)
{
	...
	{
		if(parsed[0] == "CAP" and parsed[1] == "LS")
	...
	{
		if(parsed[0] == "NICK")
			return (command_nick(client, parsed));
...
int CommandManager::command_kick(Client& client, vector<string>& parsed)
{
	...
	else if(!server->find_by_nickname(parsed[2]) or server->find_by_nickname(parsed[2])->channels.find(chan_name)==server->find_by_nickname(parsed[2])->channels.end())
	...
	else
	{
		...
		channels[chan_name]->part(channels[chan_name]->getClientByName(parsed[2]));
		client.channels.erase(chan_name);
	}
	return 0;
}
